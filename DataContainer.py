...
class DataContainer(object):
    ...
    def __init__(self, args):
        ...
        self.de_genes_by_stage = None  # Map with stages as key and simple lists of DE genes as values
        ...
        if self.args.raw_data is not None:
            ...
            self.gene_map, self.data_frame_header = self.parse_plot_data(self.args.plot_data)

        else:
            print "Anomoally.Exiting."
            sys.exit()
    ...
    def parse_htseq(self):
        ...
        with open(self.args.out + "_count_matrix.txt", 'wb+') as matrix:
    ...
    def parse_plot_data(self, datafile):
        ...
        with open(datafile, 'rb') as datafile:
            ...
            for row in data_reader:
                if rowcount == 0:
                    self.data_frame_header = row
                else:
                    self.gene_map[row[0].capitalize()] = row[1:]

                rowcount += 1
        return self.gene_map, self.data_frame_header
...
class DataPrinter:
    ...
    def write_ercc_data(self):
        with open((self.args.out + ".all_ercc.txt"), "wb+") as ercc:
    ...
    def write_plot_data(self):
        with open(self.args.out + '_plotter_data.txt', 'wb+') as plot:
    ...
    def write_filtered_data(self):
        with open((self.args.out + '_filtered.txt'), "wb+") as filt:
    ...
    def write_de_results(self):
        with open(self.args.out + "_de_gene_list.txt", 'wb+') as results_out:
